...
EOSC_BINARY = '/usr/local/bin/eosc'
...
def eosc_call(cmd):
    result = subprocess.run([EOSC_BINARY, '--eosctest', '--exec', cmd, 'attach'], stdout=subprocess.PIPE)
...
def get_addresses(start_block, end_block):
    ...
    return json.loads(eosc_call(command))
...
def main():
    ...
    sqlite.commit()
    latest_block = int(eosc_call('eth.getBlock("latest").number'))
    ...
    output = eosc_call('loadScript("%s")' % RICHLIST_TMP)[:-1]
